...
class QwenQwenHybrid(nn.Module):
    ...
    def __init__(self,
        ...
        self.vision_model.merger.mlp[2] = DtypeWrapper(5120, r1_hidden_dim)
        ...
        # Add hooks to monitor and clip gradients
        def clip_grad_hook(grad):
            # Aggressively clip gradients
            return torch.clamp(grad, min=-0.1, max=0.1)
        
        self.vision_model.merger.mlp[2].linear.weight.register_hook(clip_grad_hook)
        self.vision_model.merger.mlp[2].linear.bias.register_hook(clip_grad_hook)
